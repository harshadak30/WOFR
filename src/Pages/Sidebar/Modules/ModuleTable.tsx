...
const MultiSelectDropdown = ({
  ...
  useEffect(() => {
    ...
    setSelectedOptions(
      options.filter((opt: { selected: any }) => opt.selected)
    );
  }, [options]);
      ...
      render: (value, row) => (
        <MultiSelectDropdown
          ...
          onChange={(selected: DropdownOption[]) =>
            handleDropdownChange(row.id, "action", selected)
          }
    ...
    },
    {
      ...
      render: (value, row) => (
        <MultiSelectDropdown
          options={roleOptions}
          onChange={(selected: DropdownOption[]) =>
            handleDropdownChange(row.id, "roles", selected)
          }
      ...
      render: (value, row) => (
        <div className="flex items-center justify-center">
          <label className="inline-flex relative items-center cursor-pointer">
            ...
            />
            <div
              className={`w-11 h-6 bg-gray-200 rounded-full peer transition-colors duration-200 ease-in-out ${
                value ? "bg-blue-500" : ""
              }`}
            >
              <div
